...
import { ArrowUpRight, ArrowDownRight, Minus, RefreshCw, TrendingUp, TrendingDown } from 'lucide-react';
...
export function SilverSpreadTile() {
  const { data, isLoading, error, refetch } = useSilverSpread(60000);
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div 
              ...
              onClick={refetch}
            >
              <span className="text-xs text-destructive">Data unavailable</span>
              <RefreshCw className="w-3 h-3 text-destructive" />
            </div>
          </TooltipTrigger>
          <TooltipContent>
            <p>Click to retry fetching silver spread data</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">Silver Spread</span>
              {dataSource === 'cached' && (
        ...
        <TooltipContent side="bottom" className="max-w-xs">
          <div className="space-y-2 text-xs">
            <p className="font-medium">Shanghai vs COMEX Silver Spread</p>
            <div className="space-y-1 text-muted-foreground">
              <p><strong>COMEX:</strong> {formatPrice(comex.price)}/oz ({comex.source})</p>
              {hasShanghai ? (
                <>
                  <p><strong>Shanghai:</strong> {formatPrice(shanghai.priceUSD)}/oz (¥{shanghai.priceCNY.toLocaleString()}/kg, {shanghai.session})</p>
